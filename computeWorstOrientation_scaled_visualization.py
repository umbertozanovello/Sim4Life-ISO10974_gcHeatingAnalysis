...
temp_files_directory = r"E:\fuss\P6_STASIS_paper\temporary"
material_databases = ["User INRIM Default"]
...
def computeTemperatureWorstOrientation_allValues(T, coords):
	...
	for i,voxel_T in enumerate(T):
		eigval, eigvect = np.linalg.eigh(voxel_T)
		max_temp_values.append(eigval[2] * bField_amplitude**2)
		worst_dirs.append(eigvect[:,2] * bField_amplitude)
		if eigval[2] > maxmax_temp:
			maxmax_temp = eigval[2]
			worst_B = eigvect[:,2]
			max_T_coords = coords[i]
	...
	return worst_B, max_temp_values, worst_dirs
...
def createEMSourceFileWorstB(worst_B_dir, fname):

	bb = model.AllEntities()[model_embb_name]
	bb_size = np.array([bb.Parameters[0].Value, bb.Parameters[1].Value, bb.Parameters[2].Value])
	bb_center = np.array(bb.Transform.Translation)
	
	with open(temp_files_directory+"\\"+fname+".txt",'w') as f:
		f.write("%.3f 2 %.3f\n" %((bb_center[0]-np.ceil(bb_size[0]/2))*1e-3, np.ceil(bb_size[0])*1e-3))
		f.write("%.3f 2 %.3f\n" %((bb_center[1]-np.ceil(bb_size[1]/2))*1e-3, np.ceil(bb_size[1])*1e-3))
		f.write("%.3f 2 %.3f\n\n" %((bb_center[2]-np.ceil(bb_size[2]/2))*1e-3, np.ceil(bb_size[2])*1e-3))
		...
		B_string = f"{worst_B_dir[0]} {worst_B_dir[1]} {worst_B_dir[2]}\n"
		for _ in range(8):
			f.write(B_string)


def setThermalSimulationWorstB(simName, em_simName):
	try:
		# Creating the simulation
		del document.AllSimulations[simName]
		simulation = thermal.TransientSimulation()
		...
		# Simulation links
		link__overall_field = document.AllSimulations[em_simName].AllComponents["Overall Field"]

		# Adding a new TransientHeatSourceSettings
		transient_heat_source_settings = simulation.AddHeatSourceSettings([link__overall_field])
		...
		components = [simulation.AllComponents["Overall Field"]]
		simulation.Add(transient_field_sensor_settings, components)
		transient_field_sensor_settings.MaximumNoSnapshots = 6
...
def main():
	...
	np.save(fr'E:\fuss\P6_STASIS_paper\worst_B_power_{implant_name}.npy', worst_B_power)

	if execute_visualizations:
		# add the field direction vector for the highest deposited power
		add_worst_B_vector(worst_B_power/bField_amplitude, fname='worst_B_power')
	...
	if execute_thermal:
		if not onlyExtract:
			...
			worst_B_temp, max_temp_values, worst_dirs = computeTemperatureWorstOrientation_allValues(T, coords)

			# save the results
			np.save(fr'E:\fuss\P6_STASIS_paper\worst_B_temp_{implant_name}.npy', worst_B_temp)
			np.save(fr'E:\fuss\P6_STASIS_paper\temp_values_{implant_name}.npy', max_temp_values)
			np.save(fr'E:\fuss\P6_STASIS_paper\temp_coords_{implant_name}.npy', coords)
			np.save(fr'E:\fuss\P6_STASIS_paper\temp_worst_dirs_{implant_name}.npy', worst_dirs)

			if execute_visualizations:
				# add the field direction vector for the worst temperature
				add_worst_B_vector(worst_B_temp/bField_amplitude, fname='worst_B_temp')

				# add the scalar field of worst temperatures per voxel
				field_name = f'{implant_name}_temp_per_voxel'
				add_scalar_field(field_name, coords, max_temp_values)
				
				# create simulations for a B field with direction of worst temperature voxel
				createEMSourceFileWorstB(worst_B_temp, "worst_B_temp")
				simulation = setEMSimulation("worst_B_temp_em", "worst_B_temp.txt")
				simulation.CreateVoxels()
				simulation.RunSimulation()
				while not simulation.HasResults():
					time.sleep(1)

				setThermalSimulationWorstB("worst_B_temp_thermal", "worst_B_temp_em")
